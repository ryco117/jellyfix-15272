...
fn main() {
    ...
    for (id, album_name) in rows {
        // Use the album `Id` to get all items with this as the `ParentId`.
        let mut date_created = None;
        let r = conn.iterate(
            format!("select DateCreated from BaseItems where ParentId = '{id}'"),
            |child_row| {
                // Find the first item in the album with a non-null `DateCreated`.
        ...
        if let Err(e) = r {
            // Ignore expected `abort` error when early exiting the `DateCreated` search loop.
        ...
        if let Some(date) = date_created {
            conn.execute(format!(
                "update BaseItems set DateCreated = '{date}' where Id = '{id}'"
            ))
            .expect("Failed to update DateCreated");
